...
//convert from mpz_t to size_t to write into files.
	size_t N=(size_t)mpz_get_ui(n);
	size_t D=(size_t)mpz_get_ui(d);
	size_t E=(size_t)mpz_get_ui(e);
...
mpz_t *OUT_DATA;
OUT_DATA=malloc(sizeof(mpz_t)*input_len);

mpz_t *IN_DATA;
IN_DATA=malloc(sizeof(mpz_t)*input_len);
...
mpz_set_ui(N,n);
	...
	for (int i=0; i<input_len; i++){
		...
		mpz_set_ui(IN_DATA[i],in_data[i]);

		//modular_pow!!!
		mpz_powm(OUT_DATA[i],IN_DATA[i],D_OR_E,N);
	}
...
out_data[k]=mpz_get_ui(OUT_DATA[k]);
